...
const TABS = ["Overview", "Assessment", "My Reports", "Kick Counter", "Timeline", "Access"];
        ...
        {tab === "Overview" && (
          <div className="dash-section">
            ...
            <div className="grid-3">
              ...
              <div className="card dash-stat-card" onClick={() => result&&setTab("My Reports")} style={{cursor:result?"pointer":"default"}}>
                <div className="dsc-icon">🧠</div>
                <div className="dsc-label">Child Neuro Risk</div>
                <div className="dsc-value" style={{color: result?(result.neuro<30?"#059669":result.neuro<60?"#d97706":"#dc2626"):"var(--muted)"}}>
                  {result ? `${result.neuro}% Risk` : "Pending GDM data"}
                </div>
                <div className="dsc-cta">{result?"View full report →":"Based on GDM result"}</div>
              </div>
            </div>
              ...
              <>
                ...
                {result && (
                  <div className="card" style={{marginTop:"1.1rem",background:"#f0fdfa"}}>
                    ...
                    <ul style={{marginTop:".75rem",paddingLeft:"1.25rem",color:"var(--muted)",lineHeight:2,fontSize:".9rem"}}>
                      {result.gdm > 50 && <li>Consult your OB-GYN for a Glucose Tolerance Test (OGTT)</li>}
                      {(history[0]?.formData?.activity ?? formData.activity)==="sedentary" && <li>Aim for 30 minutes of light walking daily</li>}
                      {(history[0]?.formData?.diet ?? formData.diet)==="poor" && <li>Reduce sugar and refined carbs; consult a nutritionist</li>}
                      <li>Schedule your next prenatal check-up</li>
                      <li>Track blood glucose daily using a glucometer</li>
                    </ul>
                  </div>
                )}
