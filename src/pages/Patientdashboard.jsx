...
function validateForm(data) {
  ...
  } else if (+data.age < 15 || +data.age > 60) {
    errors.age = "Age must be 15–60";
  }

  if (!data.weight) {
    errors.weight = "Weight is required";
  } else if (+data.weight < 30 || +data.weight > 200) {
    errors.weight = "Weight must be 30–200 kg";
  }

  if (!data.height) {
    errors.height = "Height is required";
  } else if (+data.height < 100 || +data.height > 220) {
    errors.height = "Height must be 100–220 cm";
  }

  // glucose is optional — only validate if entered
  if (
    data.glucose &&
    (+data.glucose < 50 || +data.glucose > 400)
  ) {
  ...
  if (
    data.pregnancyWeek &&
    (+data.pregnancyWeek < 1 || +data.pregnancyWeek > 42)
  ) {
...
function calcRisk(data) {
  ...
  const bmi =
    data.bmi || calcBMI(data.weight, data.height);
...
function RiskGauge({ value, label }) {
  ...
  return (
    <div className="risk-gauge">
      ...
      <span
        className={`risk-badge risk-${cat.toLowerCase()}`}
      >
        {cat} Risk
      </span>
    </div>
    ...
    try {
      ...
      if (!demo && user) {
        try {
          ...
          toast.success(
            "Voice test saved to your history!"
          );
        } catch (err) {
          console.error(
            "Voice save error:",
          ...
          toast.error(
            "Voice result saved locally only."
          );
        }
      }
    } catch (err) {
      ...
      setError(
        "Unable to analyse the audio. Please try again."
      );
...
export default function PatientDashboard({
  ...
  const [voiceHistory, setVoiceHistory] =
    useState([]);
  ...
  async function fetchHistory() {
    ...
    try {
      ...
      setHistoryError(
        `Couldn't load history: ${e.message}`
      );
    ...
    if (
      ...
      if (
        ...
        setFormErrors(
          (prev) => ({
            ...prev,
            [name]: `${rule.label} must be ${rule.min}–${rule.max}${
              rule.unit
                ? " " +
                  rule.unit
                : ""
  ...
  async function handleAssess(e) {
    ...
    try {
      ...
      toast.error(
        "Couldn't save to database — result shown locally only."
      );
    } finally {
      setSaving(false);
    }
          ...
          <div className="dash-section">
            ...
            <div className="info-banner">
              ...
              <div>
                ...
                Mellitus is high blood
                glucose during
                pregnancy, affecting 1
                in 5 women (~23M
                cases/year). Early
                detection prevents
        ...
        {tab ===
          ...
          <div className="dash-section">
            ...
            >
              ...
              activity plans, clinical
              dietary guidelines, and
              healthcare specialist
              locator based on your AI
              assessment.
            </p>
